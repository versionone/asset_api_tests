...
for(const type of types) {
    test(`Update Order Attribute on Story, removing one of two owners by OIDToken (${type})`, async t => {
        const setupCommand = `
from: Story
where:
...
Scope: ${scopeName}
        ...
        let res;
        try {
            res = await assetApiPost(setupCommand);
        } catch (e) {
...
try {
    res = await assetApiPost(payload2, type);
} catch (e) {
    t.fail(`problem with request ${e.config.data}`);
}
t.is(res.status, 200, "Expected 200 OK");
